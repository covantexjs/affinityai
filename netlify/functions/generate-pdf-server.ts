...
const generateHtmlTemplate = (archetype: any, customerName?: string) => {
  const archetypeName = archetype?.name || 'Unknown Archetype';
  const archetypeTagline = archetype?.tagline || 'Your personalized relationship insights';
  const archetypeKeywords = archetype?.keywords?.map((k: any) => k.text.toLowerCase()).join(', ') || '';
  const compatibleWith = archetype?.compatibleWith?.join(' & ') || '';
  ...
  return `
    ...
    <body>
      ...
      <div class="page">
        ...
        <div class="paragraph">
          As a ${archetypeName}, you bring a unique perspective to relationships that combines ${archetypeKeywords}. 
...
const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    const { archetype, customerName } = body;

    if (!archetype) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Archetype data is required' }),
      };
    }

    // Generate the HTML content
    const htmlContent = generateHtmlTemplate(archetype, customerName);
    ...
    return {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="${archetype.name.toLowerCase().replace(/\s+/g, '-')}-love-blueprint.pdf"`,
      },
